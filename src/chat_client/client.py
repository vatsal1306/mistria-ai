...
class StreamingChatClient:
    ...
    def is_connected(self) -> bool:
        return bool(
            ...
            and getattr(self._websocket, "connected", False)
        )
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            raise ChatClientError(
                "The websocket chat backend timed out during connection setup.",
            ) from exc
        except WebSocketException as exc:
            self.disconnect()
            raise ChatClientError(
                "Could not connect to the websocket chat backend. "
                "Start main.py and verify the configured host and port.",
            ) from exc
        except OSError as exc:
            self.disconnect()
            raise ChatClientError(
                "Could not reach the websocket chat backend. "
                "Start main.py before using the Streamlit UI.",
            ) from exc
    ...
    def stream_reply(
        ...
        if not self.is_connected:
            raise ChatClientError(
                "No active websocket session. Click 'Start chat' before sending messages.",
            )
        ...
        try:
            ...
            self.disconnect()
            raise ChatClientError(
                "The websocket chat backend timed out while streaming a response.",
            ) from exc
        except WebSocketException as exc:
            self.disconnect()
            raise ChatClientError(
                "The websocket chat backend connection was interrupted. "
                "Reconnect and try again.",
            ) from exc
        except OSError as exc:
            self.disconnect()
            raise ChatClientError(
                "The websocket chat backend became unreachable. "
                "Reconnect after the backend is available.",
            ) from exc
    ...
    def _consume_ready_event(self) -> None:
        ...
        if event_type != "ready":
            self.disconnect()
            raise ChatClientError(
                "The websocket backend did not return a ready event during connection setup.",
            )
    ...
    def _consume_stream(self) -> Generator[str, None, None]:
        while True:
            ...
            if event_type == "error":
                raise ChatClientError(
                    frame.get("detail", "Unknown websocket backend error."),
                )
    ...
    def _receive_frame(self) -> dict[str, Any]:
        ...
        if not raw_frame:
            raise ChatClientError(
                "The websocket backend returned an empty frame.",
            )
        ...
        try:
            ...
            raise ChatClientError(
                "The websocket backend returned malformed JSON.",
            ) from exc
