...
class ChatService:
    ...
    async def stream_response(
        ...
        if user_id:
            ...
            if not user_info:
                logger.warning("Unknown user_id=%s, proceeding without engagement", user_id)
        ...
        if user_info and user_id:
            ...
            if hours_away >= 12:
                ...
                logger.debug(
                    "Engagement decayed after %.1fh away for %s, score=%d",
                    hours_away, user_id, session.engagement.score,
                )
    ...
    def reset_session(self, user_id: str) -> int:
        ...
        default = self.engagement_config.default_score
