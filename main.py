...
chat_service = ChatService(settings.chat, settings.engagement, runtime)
websocket_handler = WebSocketChatHandler(
    settings.api, settings.secrets, chat_service,
)
...
async def configuration_error_handler(
    _: object, exc: ConfigurationError,
) -> JSONResponse:
    return JSONResponse(
...
async def chat(body: ChatRestRequest) -> ChatRestResponse:
    result = await chat_service.run_chat_turn(
        body.user_id, body.message, body.resume_pulse,
    )
    if result is None:
        raise HTTPException(
