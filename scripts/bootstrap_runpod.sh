...
write_env_file() {
  ...
  mkdir -p "$(dirname "$ENV_FILE")" "${REPO_DIR}/data/db" "${REPO_DIR}/Logs" "${WORKSPACE_DIR}/.cache/huggingface" "${WORKSPACE_DIR}/.cache/embeddings"
...
MISTRIA_MEMORY_EXTRACTION_ENABLED=False
